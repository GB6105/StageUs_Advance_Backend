...
const validater = (field)=>{
    return (req,res,next)=>{
        try{
            ...
            if(!inputValue) throw customError(`${field} 를 입력해주세요`, 400);
...
//     if(!req.body) throw customError(`${field}를 입력해주세요`,400)
//     const fieldRegx = regx[field];
//     if(!fieldRegx.test(input))throw customError(`${field}의 형식이 올바르지 않습니다.`,400)
//     next()
// }

// const validater = (field,input,next) => {
//     if(!input) throw customError(field+"를 입력해주세요")
//     const fieldRegx = regx[field];
//     if(fieldRegx.test(input)) throw customError(field + "의 형식이 올바르지 않습니다.")
//     next()
// }
...
//         const fieldRegx = regx.field;
//         if(!fieldRegx.te st(input))throw customError(`${field}의 형식이 올바르지 않습니다.`,400)
